...
async def conversion_callback(
    callback: CallbackQuery, callback_data: ConversionCallback
):
    conv = get_conversion(DATABASE, callback_data.id)
    await callback.message.answer(
        f"{conv['amount']} {conv['from']} = {conv['result']} {conv['to']}"
    )
...
async def create_conversion(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введіть суму для конвертації:", reply_markup=ReplyKeyboardRemove()
    )
...
async def set_amount(message: Message, state: FSMContext):
    if message.text.replace(".", "", 1).isdigit():
        ...
        await message.answer(
            "Виберіть валюту, з якої конвертуємо:",
            reply_markup=currency_inline_keyboard(0, prefix="from"),
        )
    else:
...
async def currency_from_callback(call: CallbackQuery, state: FSMContext):
    ...
    if value == "next":
        await call.message.edit_reply_markup(
            reply_markup=currency_inline_keyboard(page + 1, prefix="from")
        )
    elif value == "prev":
        await call.message.edit_reply_markup(
            reply_markup=currency_inline_keyboard(page - 1, prefix="from")
        ...
        await call.message.answer(
            "Виберіть валюту, в яку конвертуємо:",
            reply_markup=currency_inline_keyboard(0, prefix="to"),
        )
    await call.answer()
...
async def currency_to_callback(call: CallbackQuery, state: FSMContext):
    ...
    if value == "next":
        await call.message.edit_reply_markup(
            reply_markup=currency_inline_keyboard(page + 1, prefix="to")
        )
    elif value == "prev":
        await call.message.edit_reply_markup(
            reply_markup=currency_inline_keyboard(page - 1, prefix="to")
